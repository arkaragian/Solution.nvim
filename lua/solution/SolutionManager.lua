...
SolutionManager.SelectBuildPlatform = function()
    ...
    local opts = {
        prompt = string.format(
            "Select Platform for [%s] Configuration [%s]:",
            SolutionManager.Solution.SolutionPath,
...
SolutionManager.SelectStartupProject = function()
    if SolutionManager.Solution == nil then
        vim.notify(
            "No solution detected. Cannot select startup project",
...
SolutionManager.Launch = function()
    ...
    for _, theProject in ipairs(SolutionManager.Solution.Projects) do
        if Current.StartupProject == theProject.Name then
            thePath = path.GetParrentDirectory(SolutionManager.Solution.SolutionPath, osutils.seperator)
                .. osutils.seperator
                .. theProject.RelPath
            break
        end
    ...
    if Current.StartupLaunchProfile == nil then
        ...
        command = string.format(
            "!start cmd /K dotnet run --project %s --launch-profile %s -c %s",
            thePath,
            Current.StartupLaunchProfile,
            Current.BuildConfiguration
        )
    end
...
local function GetLaunchProfileJson()
    ...
    for _, v in ipairs(SolutionManager.Solution.Projects) do
        if Current.StartupProject == v.Name then
            startupProjectPath = path.GetParrentDirectory(SolutionManager.Solution.SolutionPath, osutils.seperator)
                .. osutils.seperator
                .. v.RelPath
            launchSettingsLocation = path.GetParrentDirectory(startupProjectPath, osutils.seperator)
                .. osutils.seperator
                .. "Properties"
                .. osutils.seperator
                .. "launchSettings.json"
    ...
    elseif #Current.OutputLocations > 1 then
        ...
        if the_profile == nil then
            vim.notify(
                "Could not locate "
                    .. Current.StartupLaunchProfile
                    .. " in launchSettings.json returning sniffed compile location",
                vim.log.levels.ERROR,
                { title = "Solution.nvim" }
            )
            return Current.OutputLocations[1].OutputLocation
